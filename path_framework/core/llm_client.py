...
class OpenAIClient(BaseLLMClient):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise PathFrameworkError("OpenAI library not installed. Run: pip install openai")
        except Exception as e:
            logger.error(f"OpenAI API error: {e}")
            raise PathFrameworkError(f"OpenAI generation failed: {str(e)}")
...
Ensure your response is valid JSON only, no additional text."""
        ...
        try:
            ...
            raise PathFrameworkError(f"Invalid JSON response from LLM: {str(e)}")
...
class AnthropicClient(BaseLLMClient):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise PathFrameworkError("Anthropic library not installed. Run: pip install anthropic")
        except Exception as e:
            logger.error(f"Anthropic API error: {e}")
            raise PathFrameworkError(f"Anthropic generation failed: {str(e)}")
...
class OllamaClient(BaseLLMClient):
    ...
    def __init__(self, api_key: str = "", model: str = "llama2", base_url: str = "http://localhost:11434", timeout: int = 60):
        super().__init__(api_key, model, timeout)
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise PathFrameworkError("aiohttp library not installed. Run: pip install aiohttp")
        except Exception as e:
            logger.error(f"Ollama API error: {e}")
            raise PathFrameworkError(f"Ollama generation failed: {str(e)}")
...
class OpenRouterClient(BaseLLMClient):
    ...
    async def generate(self, request: LLMRequest) -> LLMResponse:
        ...
        try:
            ...
            raise PathFrameworkError(f"OpenRouter generation failed: {str(e)}")
...
class LLMClientFactory:
    ...
    @staticmethod
    def create_client(provider: LLMProvider, api_key: str, model: str, **kwargs) -> BaseLLMClient:
    ...
    def create_from_config(config: Dict[str, Any]) -> BaseLLMClient:
        ...
        api_key = config.get("api_key") or os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY") or os.getenv("OPENROUTER_API_KEY")
        model = config.get("model", "gpt-4")
        
        if not api_key and provider in [LLMProvider.OPENAI, LLMProvider.ANTHROPIC, LLMProvider.OPENROUTER]:
...
def get_llm_client(provider: str = None, api_key: str = None, model: str = None, phase: int = None, **kwargs) -> BaseLLMClient:
    """
...
def _get_fallback_config(provider: str = None, api_key: str = None, model: str = None, phase: int = None, **kwargs) -> Dict[str, Any]:
    """Fallback configuration using environment variables only"""
    
    provider = provider or os.getenv("PATH_LLM_PROVIDER", "openai")
    api_key = api_key or os.getenv("OPENAI_API_KEY") or os.getenv("ANTHROPIC_API_KEY") or os.getenv("OPENROUTER_API_KEY")
