...
class PathConfig:
        ...
        **kwargs,
        ...
        if not explicit_config.get("model") and not env_config.get("model"):
            ...
            if provider_val == "openai":
                ...
                final_config["model"] = final_config.get(
        ...
        if not final_config.get("api_key") and final_config.get("provider") != "ollama":
            logger.warning(
                f"No API key found for provider: {final_config.get('provider')}"
            )

        logger.debug(f"Final LLM config: {self._mask_sensitive(final_config)}")
        return final_config

    def _get_phase_specific_model(self, phase: int | None = None) -> str | None:
        """
    ...
    def _get_env_int(self, key: str) -> int | None:
    ...
    def _get_env_float(self, key: str) -> float | None:
    ...
    def create_config_file(self, path: str, template: bool = True) -> None:
        ...
        config_template = {
            ...
            "agents": {
                "domain_analyst": {"confidence_threshold": 0.8, "phase": 1},
                "system_architect": {
                    "technology_preference": "cloud_native",
                    "phase": 1,
                },
                "component_designer": {"solid_principles": True, "phase": 1},
