...
from ....core.llm_client import LLMRequest, get_llm_client
...
class DomainAnalysisResult:
    ...
    stakeholders: StakeholderAnalysis | None = None
...
class AIDomainAnalyst(BaseAgent):
    ...
    async def analyze_requirements(
        self, request: DomainAnalysisRequest
        ...
        try:
            self.logger.info(
                f"Starting requirements analysis for {request.project_name}"
            )

            # Validate input
            if not request.project_description or not request.business_context:
                raise PathFrameworkError(
                    "Project description and business context are required"
                )
            ...
            human_review = self._requires_human_review(
                classified_requirements, confidence
            )
            ...
            self.logger.info(
                f"Requirements analysis completed with {len(classified_requirements)} requirements"
            )
            return result

        except Exception as e:
            self.logger.error(f"Stakeholder analysis failed: {e!s}")
            raise PathFrameworkError(f"Stakeholder analysis failed: {e!s}")
    ...
    async def model_domain(
        ...
        try:
            ...
            entities = await self._extract_domain_entities(
                request.project_description, request.business_context
            )

            # Model relationships
            await self._model_relationships(entities)
            ...
            result = DomainAnalysisResult(
                ...
                recommendations=await self._generate_domain_model_recommendations(
                    domain_model
                ),
            )
            ...
            self.logger.error(f"Domain modeling failed: {e!s}")
            raise PathFrameworkError(f"Domain modeling failed: {e!s}")
    ...
    async def extract_business_rules(
        ...
        try:
            ...
            result = DomainAnalysisResult(
                ...
                recommendations=await self._generate_business_rules_recommendations(
                    validated_rules
                ),
            )

            self.logger.info(
                f"Business rules extraction completed with {len(validated_rules)} rules"
            )
            return result

        except Exception as e:
            self.logger.error(f"Business rules extraction failed: {e!s}")
            raise PathFrameworkError(f"Business rules extraction failed: {e!s}")
    ...
    async def analyze_stakeholders(
        ...
        try:
            ...
            stakeholder_analysis = await self._analyze_stakeholders(
                request.stakeholder_input, request.business_context
            )
            ...
            result = DomainAnalysisResult(
                ...
                recommendations=await self._generate_stakeholder_recommendations(
    ...
    async def _extract_requirements(
        self, description: str, context: str, stakeholder_input: list[str]
    ) -> list[Requirement]:
...
{{
  "requirements": [
    {{
      ...
      "stakeholders": ["string"]
            ...
            try:
                ...
                return await self._fallback_extract_requirements(
                    description, context, stakeholder_input
                )
    ...
    async def _fallback_extract_requirements(
        self, description: str, context: str, stakeholder_input: list[str]
    ) -> list[Requirement]:
        ...
        for i, sentence in enumerate(sentences):
            ...
            if any(
                indicator in sentence.lower() for indicator in requirement_indicators
            ):
                req = Requirement(
    ...
    async def _classify_requirements(
        ...
        for req in requirements:
            ...
            req.acceptance_criteria = await self._generate_acceptance_criteria(
                req.description
            )
    ...
    def _classify_requirement_type(self, text: str) -> RequirementType:
        ...
        if any(
            word in text_lower
            for word in ["performance", "security", "scalability", "availability"]
        ):
            return RequirementType.NON_FUNCTIONAL
        elif any(
            word in text_lower
            for word in ["business", "process", "workflow", "compliance"]
        ):
            return RequirementType.BUSINESS
        elif any(
            word in text_lower
            for word in ["technical", "database", "api", "integration"]
        ):
    ...
    async def _model_relationships(
        ...
        for entity in entities:
            for other_entity in entities:
                if entity.name != other_entity.name:
                    ...
                    if (
                        "user" in entity.name.lower()
                        and "order" in other_entity.name.lower()
                    ):
                        relationships[f"{entity.name}_to_{other_entity.name}"] = (
                            "one_to_many"
                        )

        return relationships

    async def _identify_bounded_contexts(
        self, entities: list[DomainEntity]
    ) -> list[str]:
        ...
        for entity in entities:
            if any(
                word in entity.name.lower() for word in ["user", "customer", "account"]
            ):
                contexts.add("User Management")
            elif any(
                word in entity.name.lower() for word in ["order", "product", "payment"]
            ):
                contexts.add("Order Management")
            elif any(
                word in entity.name.lower() for word in ["report", "data", "analytics"]
            ):
                contexts.add("Analytics")

        return list(contexts)

    async def _identify_domain_services(
        self, entities: list[DomainEntity]
    ) -> list[str]:
    ...
    async def _extract_business_rules(
        ...
        rule_patterns = [("if", "then"), ("when", "must"), ("should", "unless")]
            ...
            for condition_word, action_word in rule_patterns:
                if condition_word in sentence and action_word in sentence:
                    rule = BusinessRule(
                        ...
                        description=sentence,
                        condition=sentence.split(condition_word)[1]
                        .split(action_word)[0]
                        .strip(),
    ...
    async def _analyze_stakeholders(
        self, stakeholder_input: list[str], context: str
    ) -> StakeholderAnalysis:
        ...
        if stakeholder_input:
            for input_text in stakeholder_input:
                ...
                if any(
                    ...
                    for word in ["user", "customer", "client"]
                ):
                    primary_stakeholders.append("End Users")
                elif any(
                    word in input_text.lower()
                    for word in ["business", "manager", "owner"]
                ):
                    primary_stakeholders.append("Business Stakeholders")
                elif any(
                    word in input_text.lower()
                    for word in ["developer", "technical", "admin"]
                ):
    ...
    def _calculate_confidence(
        self, requirements: list[Requirement], request: DomainAnalysisRequest
    ) -> float:
        ...
        completeness_score = min(
            len(requirements) / 10.0, 1.0
        )  # Assume 10 requirements is complete
        clarity_score = sum(
            1 for req in requirements if len(req.description) > 20
        ) / len(requirements)
    ...
    def _calculate_domain_model_confidence(self, domain_model: DomainModel) -> float:
        ...
        entity_score = min(
            len(domain_model.entities) / 5.0, 1.0
        )  # Assume 5 entities is good
        context_score = min(
            len(domain_model.bounded_contexts) / 3.0, 1.0
        )  # Assume 3 contexts is good
    ...
    def _requires_human_review(
        self, requirements: list[Requirement], confidence: float
    ) -> bool:
        ...
        critical_requirements = [
            req for req in requirements if req.priority == RequirementPriority.CRITICAL
        ]
        business_requirements = [
            req for req in requirements if req.type == RequirementType.BUSINESS
        ]
    ...
    def _assess_complexity(self, description: str) -> float:
        ...
        score = sum(
            1 for indicator in complexity_indicators if indicator in description.lower()
        )
    ...
    async def _generate_requirements_recommendations(
        self, requirements: list[Requirement]
    ) -> list[str]:
        ...
        if not requirements:
            recommendations.append(
                "No requirements identified. Consider conducting stakeholder interviews."
            )

        critical_count = len(
            [r for r in requirements if r.priority == RequirementPriority.CRITICAL]
        )
        if critical_count > 5:
            recommendations.append(
                "High number of critical requirements. Consider prioritization review."
            )

        complex_requirements = [r for r in requirements if r.complexity_score > 0.7]
        if complex_requirements:
            recommendations.append(
                f"Found {len(complex_requirements)} high-complexity requirements. Consider breaking them down."
            )

        return recommendations

    async def _generate_domain_model_recommendations(
        ...
        if len(domain_model.entities) < 3:
            recommendations.append(
                "Consider identifying more domain entities for a complete model."
            )

        if not domain_model.bounded_contexts:
            recommendations.append(
                "Define bounded contexts to better organize the domain."
            )

        if len(domain_model.domain_services) < 2:
            recommendations.append(
                "Consider identifying domain services for business logic encapsulation."
            )

        return recommendations

    async def _generate_business_rules_recommendations(
        self, rules: list[BusinessRule]
    ) -> list[str]:
        ...
        if not rules:
            recommendations.append(
                "No business rules identified. Review requirements for implicit business logic."
            )

        incomplete_rules = [r for r in rules if not r.condition or not r.action]
        if incomplete_rules:
            recommendations.append(
                f"Found {len(incomplete_rules)} incomplete business rules. Review and complete them."
            )

        return recommendations

    async def _generate_stakeholder_recommendations(
        self, analysis: StakeholderAnalysis
    ) -> list[str]:
        ...
        recommendations.append(
            "Schedule regular stakeholder reviews throughout the project."
        )
        ...
        try:
            ...
            if result.business_rules:
                response["business_rules"] = [
                    asdict(rule) for rule in result.business_rules
                ]
            ...
            self.logger.error(f"Domain analysis execution failed: {e!s}")
            raise PathFrameworkError(f"Domain analysis execution failed: {e!s}")
    ...
    def validate_output(self, output: dict[str, Any]) -> bool:
        ...
        return output["analysis_type"] in valid_types
