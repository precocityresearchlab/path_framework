...
from ..agents_base import AgentStatus, AgentTask
...
class BasePhase(ABC):
    ...
    async def start_phase(self, input_data: dict[str, Any]) -> PhaseResult:
        ...
        if self.status != PhaseStatus.NOT_STARTED:
            raise PhaseError(
                f"Phase {self.phase_id} already started (status: {self.status})"
            )
    ...
    async def assign_task_to_agent(
        self, agent_capability: str, task: AgentTask
    ) -> UUID:
        """
    ...
    async def wait_for_task_completion(
        self, task_id: UUID, timeout: float | None = None
    ) -> dict[str, Any]:
        """
    ...
    def update_progress(self, current_step: int, total_steps: int):
        ...
        self.logger.debug(
            f"Phase progress: {progress:.1f}% ({current_step}/{total_steps})"
        )
