...
async def demo_real_llm_integration():
    ...
    if openai_key:
        provider = "openai"
        model = "gpt-4"
        print("🤖 Using OpenAI GPT-4 for real LLM integration")
    else:
        provider = "anthropic"
        model = "claude-3-sonnet-20240229"
        print("🤖 Using Anthropic Claude for real LLM integration")
    ...
    try:
        ...
        domain_analyst = AIDomainAnalyst(
            config={"llm_provider": provider, "llm_model": model}
        )
        ...
        if analysis_result.requirements:
            ...
            for i, req in enumerate(analysis_result.requirements, 1):
                ...
                if req.acceptance_criteria:
                    print(
                        f"   Acceptance Criteria: {len(req.acceptance_criteria)} criteria defined"
                    )
        ...
        print("   • AI-powered requirements extraction")
        print("   • Structured data parsing from LLM")
        print("   • Project artifact generation")
