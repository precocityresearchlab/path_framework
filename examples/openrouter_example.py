...
from path_framework.phases.arch.ai.domain_analyst import (
    AIDomainAnalyst,
    AnalysisType,
...
async def demo_openrouter_integration():
    ...
    try:
        ...
        for model in models_to_test:
            ...
            try:
                ...
                client = LLMClientFactory.create_client(
                    provider=LLMProvider.OPENROUTER, api_key=openrouter_key, model=model
                )
        ...
        domain_analyst = AIDomainAnalyst(
            config={"llm_provider": "openrouter", "llm_model": "openai/gpt-4"}
        )
        ...
        print("🔄 Running requirements analysis with OpenRouter...")
        ...
        for model in alternative_models:
            try:
                ...
                client = get_llm_client(
                    provider="openrouter", api_key=openrouter_key, model=model
                )

                request = LLMRequest(
                    prompt=simple_prompt, temperature=0.1, max_tokens=100
                )
        ...
        print("✅ Demonstrated:")
        print("   • Multiple model access via single API")
        print("   • PATH Framework agent integration")
        print("   • Model switching capabilities")
        print("   • Requirements analysis with AI")
...
async def demo_openrouter_structured_generation():
    ...
    try:
        ...
        client = get_llm_client(
            provider="openrouter", api_key=openrouter_key, model="openai/gpt-4"
        )
